...
// Renders Gemini markdown: ### headings, **bold**, *italic*, - bullets, newlines
...
function FormattedText({ text }) {
  ...
  while (i < lines.length) {
    ...
    // Skip empty lines — add a small gap
    ...
    if (/^###\s+/.test(trimmed)) {
      ...
      elements.push(
        <div key={i} style={{ fontFamily: "var(--font-display)", fontSize: 13, fontWeight: 700, color: "var(--green-bright)", letterSpacing: ".04em", marginTop: 10, marginBottom: 2 }}>
          {inlineParse(txt)}
        </div>
      );
      i++; continue;
    ...
    if (/^##\s+/.test(trimmed)) {
      const txt = trimmed.replace(/^##\s+/, "");
      elements.push(
        <div key={i} style={{ fontFamily: "var(--font-display)", fontSize: 14, fontWeight: 800, color: "var(--green)", letterSpacing: ".03em", marginTop: 12, marginBottom: 3 }}>
          {inlineParse(txt)}
        </div>
      );
      i++; continue;
    ...
    if (/^#\s+/.test(trimmed)) {
      const txt = trimmed.replace(/^#\s+/, "");
      elements.push(
        <div key={i} style={{ fontFamily: "var(--font-display)", fontSize: 15, fontWeight: 800, color: "var(--green)", letterSpacing: ".02em", marginTop: 14, marginBottom: 4 }}>
          {inlineParse(txt)}
        </div>
      );
      i++; continue;
    ...
    if (/^[-*•]\s+/.test(trimmed)) {
      ...
      elements.push(
        <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
          ...
          <span style={{ fontFamily: "var(--font-body)", fontSize: 13, lineHeight: 1.75, color: "var(--text-dim)" }}>{inlineParse(txt)}</span>
        </div>
      );
      i++; continue;
    }
    ...
    // Normal paragraph
    elements.push(
      <p key={i} style={{ margin: 0, fontFamily: "var(--font-body)", fontSize: 13, lineHeight: 1.8, color: "var(--text-dim)" }}>
        {inlineParse(trimmed)}
      </p>
    );
    i++;
...
export default function AnalyzePage({ onNav }) {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      setChat([{
        role: "ai",
        content: `Analysis complete! 🌾 ML predicts **${parsed.predictionClass}** as your best crop (${parsed.probability}% confidence). Food security score: ${parsed.cropScore}/100 — ${parsed.riskLevel} Score. Ask me anything!`,
      }]);
    ...
    <div className="shell">
      ...
      <div className="main-scroll">
        <div className="main-inner">
          ...
          {/* ── RESULTS ── */}
          {step === "done" && result && (
            <div className="results-cols" style={{ animation: "fadeUp .5s ease", height: "calc(100vh - 160px)", minHeight: 600 }}>

              {/* Left — results */}
              <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: "clamp(16px,3.5vw,28px)", display: "flex", flexDirection: "column", overflow: "hidden", minHeight: 0 }}>
                ...
                {result.aiResponse && (
                  <div style={{ background: "rgba(61,107,32,.1)", border: "1px solid rgba(122,171,68,.22)", borderRadius: 10, padding: "16px 18px" }}>
                    ...
                    <div style={{ maxHeight: 220, overflowY: "auto", paddingRight: 6 }}><FormattedText text={result.aiResponse} /></div>
                  </div>
                )}
                </div>{/* end scrollable */}
              </div>
              ...
              {/* Right — Gemini chat */}
              <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: "clamp(16px,3.5vw,24px)", display: "flex", flexDirection: "column", overflow: "hidden", minHeight: 0 }}>
                ...
                <div style={{ flex: 1, overflowY: "auto", marginBottom: 14, display: "flex", flexDirection: "column", gap: 10, minHeight: 0 }}>
                  {chat.map((m, i) => (
